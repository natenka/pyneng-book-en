...
Method shutdown() indicates that Executor object must be finished. However, if to shutdown() method pass ``wait=True`` (default value),
it will not return the result until all functions running in threads have been completed.
If ``wait=False``, shutdown() method returns instantly but script itself will not exit until all functions have been completed.
